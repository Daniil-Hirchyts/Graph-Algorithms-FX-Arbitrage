...
import { DFSRequestSchema, type DFSRequest, DFSResponseSchema } from '@/lib/schemas';
...
export function DfsForm() {
  ...
  const form = useForm<DFSRequest>({
    resolver: zodResolver(DFSRequestSchema),
    defaultValues: {
      start_node: '',
    },
  });

  const onSubmit = async (data: DFSRequest) => {
    try {
      setLoading(true);
      setError(null);
      if (!loadedGraph) {
      ...
      setError(err instanceof Error ? err.message : 'Failed to run DFS');
    } finally {
      setLoading(false);
    }
  };
